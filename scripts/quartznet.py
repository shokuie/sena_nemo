...
def custom_checkpoint_callback(folder, load_from_folder, step_freq, bucket_name):
    ret = nemo.core.CheckpointCallback(folder, load_from_folder, step_freq)

    check_point_dir = folder
    check_point_zip_file = "checkpoint"
    extension = "zip"
    shutil.make_archive(check_point_dir + check_point_zip_file, extension, check_point_dir)
    file_name_to_copy = check_point_dir + check_point_zip_file + "." + extension  # name of the file to upload
    s3_client = boto3.client('s3')
    s3_client.upload_file(file_name_to_copy, bucket_name, check_point_zip_file + "." + extension)
    print("Checkpoint backup done")
    return ret
...
def create_all_dags(args, neural_factory):
    ...
    if args.checkpoint_dir or args.load_dir:
        chpt_callback = custom_checkpoint_callback(folder=args.checkpoint_dir, load_from_folder=args.load_dir, step_freq=args.checkpoint_save_freq, bucket_name=args.bucket_name)
